...
ifeq (,$(filter clean cleandeps cleandll,$(MAKECMDGOALS)))
    $(call REQUIRE_VARS,RBJITGLUE_DIR)
    $(call REQUIRE_VARS,RELATIVE_OMR_DIR)
endif
...
#
# These makefiles assume a common root. I'm going to make them work assuming rbjitglue 
# and omr are inside the ruby VM directory.
JIT_SRCBASE?=$(RUBY_DIR)
JIT_OBJBASE?=$(RBJITGLUE_DIR)/objs/ruby_$(BUILD_CONFIG)
...
# in the filesystem :)
#
JIT_OMR_DIRTY_DIR?=$(RELATIVE_OMR_DIR)/compiler
JIT_PRODUCT_DIR?=$(RBJITGLUE_DIR)/ruby

#
# Dirs used internally by the makefiles
#
JIT_MAKE_DIR?=$(FIXED_SRCBASE)/$(RBJITGLUE_DIR)/ruby/build
JIT_SCRIPT_DIR?=$(FIXED_SRCBASE)/$(RBJITGLUE_DIR)/ruby/build/scripts
