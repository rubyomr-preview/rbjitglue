...
void *
Ruby::Monitor::operator new(size_t size)
   {
   return TR::globalAllocator().allocate(size);
   }

void
Ruby::Monitor::operator delete(void *p)
   {
   TR::globalAllocator().deallocate(p, sizeof(TR::Monitor));
   }

TR::Monitor *
Ruby::Monitor::create(char *name)
   {
   return new TR::Monitor(name);
   }

Ruby::Monitor::Monitor(char const *name) :
      _name(name)
   {
